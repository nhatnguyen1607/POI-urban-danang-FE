...
} from '../../services/reviewImageUpload';
import {
...
const uiCopy = {
  vi: {
    ...
    uploadFailed: 'Chưa thể tải ảnh lên. Nội dung đánh giá vẫn được giữ lại để bạn thử lại.',
    uploadingImages: 'Đang tải ảnh...',
    savingReview: 'Đang lưu đánh giá...',
    retrySubmit: 'Thử gửi lại',
    purposes: {
    ...
    uploadFailed: 'The images could not be uploaded. Your review is still here so you can retry.',
...
function ReviewModal({
  ...
  const [submitProgress, setSubmitProgress] = useState<ReviewUploadProgress | null>(null);
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    setError('');
    let currentPhase: ReviewUploadProgress['phase'] = files.length ? 'uploading' : 'saving';
    setSubmitProgress({
      phase: currentPhase,
      completedFiles: 0,
    ...
    submissionIdRef.current ||= createReviewSubmissionId();
    try {
      ...
      setError(currentPhase === 'uploading'
        ? copy.uploadFailed
        : copy.submitFailed);
    } finally {
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <label className="inline-flex cursor-pointer items-center gap-2 rounded-xl bg-slate-100 px-4 py-2 text-sm font-semibold text-slate-800 hover:bg-slate-200">
              ...
              <input
                ...
                disabled={submitting}
                className="hidden"
                onChange={(event) => setFiles(dedupeReviewFiles(Array.from(event.target.files || [])).slice(0, 6))}
              />
            </label>
            <div className="flex gap-2">
              <button onClick={onSkip} disabled={submitting} className="rounded-xl bg-slate-100 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-200 disabled:opacity-60">
                {copy.skip}
              </button>
              <button onClick={submit} disabled={submitting} className="inline-flex items-center gap-2 rounded-xl bg-cyan-400 px-4 py-2 text-sm font-semibold text-slate-950 hover:bg-cyan-300 disabled:opacity-60">
                ...
                {error ? copy.retrySubmit : copy.submit}
              </button>
            </div>
          </div>
          {submitProgress && (
            <div className="space-y-1" role="status" aria-live="polite">
              <p className="text-sm font-medium text-cyan-800">
                {submitProgress.phase === 'uploading'
                  ? copy.uploadingImages + ' ' + submitProgress.percent + '%'
                  : copy.savingReview}
              </p>
              {submitProgress.phase === 'uploading' && (
                <div className="h-1.5 overflow-hidden rounded-full bg-slate-200">
                  <div className="h-full bg-cyan-500 transition-[width]" style={{ width: submitProgress.percent + '%' }} />
                </div>
              )}
            </div>
          )}
